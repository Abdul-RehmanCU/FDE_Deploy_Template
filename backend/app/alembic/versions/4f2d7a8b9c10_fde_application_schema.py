...
def upgrade() -> None:
    ...
    op.create_check_constraint(
        "ck_user_role", "user", "role IN ('admin','operator','viewer')"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "existing_contact_count", sa.Integer(), server_default="0", nullable=False
        ),
        ...
        sa.CheckConstraint(
            "status IN ('uploaded','mapped','validating','validated','importing','completed','failed','cancelled')",
            name="ck_import_batch_status",
        ),
        sa.CheckConstraint(
            "total_rows >= 0 AND accepted_count >= 0 AND rejected_count >= 0 AND duplicate_count >= 0 AND existing_contact_count >= 0 AND inserted_count >= 0 AND skipped_count >= 0",
            name="ck_import_batch_counts",
        ),
    )
    op.create_index(
        "ix_import_batch_status_created", "import_batch", ["status", "created_at"]
    )
    ...
    op.create_table(
        ...
        *timestamps(),
        ...
        sa.CheckConstraint(
            "outcome IN ('accepted','invalid','file_duplicate','existing_contact')",
            name="ck_validation_row_outcome",
        ),
    )
    op.create_index(
        "ix_validation_row_import_outcome_row",
        "validation_row",
        ["import_id", "outcome", "row_number"],
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "attempt_count >= 0 AND max_attempts > 0", name="ck_job_attempt_counts"
        ),
    )
