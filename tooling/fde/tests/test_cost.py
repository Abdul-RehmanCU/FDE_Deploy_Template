...
def test_committed_demo_estimate_reconciles_below_cap() -> None:
    ...
    assert gate.estimated_total_usd == Decimal("4.50619304")
...
def test_rejects_understated_total(tmp_path: Path) -> None:
    text = ESTIMATE.read_text(encoding="utf-8").replace("estimated_total_usd: 4.50619304", "estimated_total_usd: 4.00")
