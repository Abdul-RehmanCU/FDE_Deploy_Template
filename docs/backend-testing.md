...
Current backend evidence is [GitHub Actions run 35713299350](https://github.com/Abdul-RehmanCU/FDE_Deploy_Template/actions/runs/35713299350)
at `4600d98`: Ruff, Mypy, Alembic, 50 PostgreSQL/Redis tests, migration-aware
readiness, the additive legacy-data upgrade, encoded database URL handling, and
the version-matched `pg_dump`/`pg_restore` drill all passed. The restore target
was the generated disposable database `fde_restore_35713299350_2161` and was
removed by the script. Other jobs in that workflow have separate acceptance
status; the backend job does not prove GKE or observability criteria.
